...
// Compatibility endpoint. All source checks use the MVP 1.7 canonical pipeline.
module.exports = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      note: 'Compatibility route backed by the canonical source → bounded content → availability evidence → PH eligibility evidence → history → status pipeline.'
    });
  } catch (error) {
