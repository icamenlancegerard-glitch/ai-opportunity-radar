// MVP 1.6 — canonical source recheck pipeline.
// source policy → bounded content → availability evidence → snapshot → history → status.
...
async function checkSource(rawUrl) {
  ...
  try {
    const response = await fetch(target.toString(), {
      ...
      redirect: 'manual',
      headers: {
        'User-Agent': 'AI-Opportunity-Radar-Recheck/1.6',
        'Range': 'bytes=0-' + (MAX_CONTENT_BYTES - 1)
      }
    });

    const bodyText = await readBoundedText(response);
    const availabilityEvidence = classifyAvailability(target.toString(), bodyText);
