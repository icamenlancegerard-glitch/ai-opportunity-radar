...
function getSupabaseConfig(env = process.env) {
  ...
  // Supabase's newer `secret` key is preferred. Keep legacy service_role support
  // for existing projects while Supabase transitions key types.
  const secretKey = typeof env.RADAR_SUPABASE_SECRET_KEY === 'string'
    ? env.RADAR_SUPABASE_SECRET_KEY.trim()
    : '';
  ...
  return { baseUrl: parsed.toString().replace(/\/+$/, ''), serviceRoleKey: apiKey };

}

function createSupabaseRestClient({ baseUrl, serviceRoleKey }) {
  const config = getSupabaseConfig({ RADAR_SUPABASE_URL: baseUrl, RADAR_SUPABASE_SECRET_KEY: serviceRoleKey });
