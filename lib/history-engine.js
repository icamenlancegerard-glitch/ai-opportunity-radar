// MVP 1.8 — portable, deterministic history engine.

function normalizeEvidence(input, allowed) {
...
function normalizeAvailabilityEvidence(input) {
  return normalizeEvidence(input, new Set(['OPEN_EVIDENCE', 'CLOSED_EVIDENCE', 'CONFLICTING_EVIDENCE', 'NOT_VERIFIED']));
}
function normalizeEligibilityEvidence(input) {
  return normalizeEvidence(input, new Set(['PH_ELIGIBLE_EVIDENCE', 'PH_EXCLUDED_EVIDENCE', 'CONFLICTING_EVIDENCE', 'NOT_VERIFIED']));
}
function normalizeCompensationEvidence(input) {
  return normalizeEvidence(input, new Set(['COMPENSATION_EVIDENCE', 'NOT_VERIFIED']));
}
...
function normalizeSnapshot(input = {}) {
  ...
  return {
    ...
    eligibilityEvidence: normalizeEligibilityEvidence(source.eligibilityEvidence),
    compensationEvidence: normalizeCompensationEvidence(source.compensationEvidence)
  };
}
...
function diffSnapshots(previousInput, currentInput) {
  ...
  if (
    previous.eligibilityEvidence.status !== current.eligibilityEvidence.status ||
    JSON.stringify(previous.eligibilityEvidence.matchedSignals) !== JSON.stringify(current.eligibilityEvidence.matchedSignals)
  ) changes.push('ELIGIBILITY_EVIDENCE_CHANGED');
  if (
    previous.compensationEvidence.status !== current.compensationEvidence.status ||
    JSON.stringify(previous.compensationEvidence.matchedSignals) !== JSON.stringify(current.compensationEvidence.matchedSignals)
  ) changes.push('COMPENSATION_EVIDENCE_CHANGED');
...
module.exports = {
  normalizeSnapshot, diffSnapshots, makeHistoryRecord,
  normalizeAvailabilityEvidence, normalizeEligibilityEvidence, normalizeCompensationEvidence
};
