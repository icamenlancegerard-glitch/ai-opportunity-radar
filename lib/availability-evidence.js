...
// Reports explicit page-language signals only; non-success HTTP responses are not
...
const RULES = {
  ...
  'ph.jobstreet.com': {
    open: ['apply now', 'active job', 'accepting applications', 'be among the first'],
  ...
  'ph.linkedin.com': {
    open: ['easy apply', 'be among the first 25 applicants', 'actively recruiting'],
...
function classifyAvailability(rawUrl, bodyText, httpStatus) {
  let hostname = null;
  ...
  if (open.length && closed.length) {
    return { status: 'CONFLICTING_EVIDENCE', source: 'explicit_page_text', matchedSignals: [...new Set(matched)] };
  }
  if (open.length) {
    return { status: 'OPEN_EVIDENCE', source: 'explicit_page_text', matchedSignals: [...new Set(open.map(signal => 'OPEN:' + signal))] };
  }
  if (closed.length) {
    return { status: 'CLOSED_EVIDENCE', source: 'explicit_page_text', matchedSignals: [...new Set(closed.map(signal => 'CLOSED:' + signal))] };
  }
