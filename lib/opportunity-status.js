// MVP 1.7 — deterministic opportunity status from source evidence only.
...
function makeOpportunityStatus(snapshot, previous, history) {
  ...
  return {
    ...
    availability: new Set(['OPEN_EVIDENCE', 'CLOSED_EVIDENCE', 'CONFLICTING_EVIDENCE']).has(availability)
      ? availability : 'NOT_VERIFIED',
    eligibility: new Set(['PH_ELIGIBLE_EVIDENCE', 'PH_EXCLUDED_EVIDENCE', 'CONFLICTING_EVIDENCE']).has(eligibility)
